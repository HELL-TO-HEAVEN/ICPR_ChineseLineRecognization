...
if __name__ == '__main__':
	...
	for imgfile in os.listdir(img_dir):
		i=i+1
		img = Image.open(img_dir+'/'+imgfile).convert('RGB')
		...
		with open(text_dir+'/'+txtname,'r',encoding="utf-8") as f:
			...
			for line in f.readlines():
				...
				name = imgname + str(i) +'_' +'.jpg'
				if w>h:
					img_new.save(os.path.join(save_dir_hor, name))
					fh.write(name+' '+label+'\n')
				else:
					img_new.save(save_dir_ver+name)
					fv.write(name+' '+label+'\n')
		# break
	fh.close()
	fv.close()
		# img = image.open(img_dir+file)
		# img_new = image.crop((128,128,256,256))
		# img_new.save(save_dir+file)
